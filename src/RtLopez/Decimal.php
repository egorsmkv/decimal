...
abstract class Decimal
{
    ...
    {
        if ($prec < 0 || self::$defaultPrecision < 0) {
     ...
     * @param int|float|double $value
     * @param string $prec
     * @return \RtLopez\Decimal|int|float|double
     */
    ...
    {
        if ($precision < 0) {
            throw new \RuntimeException('Precision can\'t be negative');
        }
    ...
    public function format($prec = null, $dec_point = '.', $thousands_sep = ' ', $trailing_zero = true)
    {
        ...
        for ($i = $int_len - 1; $i >= 0; $i--) {
            ...
            if (($int_len - $i) % 3 === 0) {
                array_unshift($int_str, $thousands_sep);
            }
        }
     ...
     * @param int|float|string|Decimal $op operand
     * @return Decimal
     * @throws \RtLopez\ArithmeticException
     */
    public function div($op)
    {
        ...
        if ($op->eq(0)) {
            throw new ArithmeticException(sprintf('Division by zero (%s)', json_encode($op)));
        }
    ...
    /**
     * Calculate epsilon for float comparision
    ...
    {
        return $this->ge(0) ? $this->floor() : $this->ceil();
    }
